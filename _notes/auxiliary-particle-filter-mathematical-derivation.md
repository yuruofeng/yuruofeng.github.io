---
title: "辅助粒子滤波：数学理论与推导详解"
date: 2025-02-07
category: "Mathematics / Statistics"
icon: "📈"
description: "辅助粒子滤波（Auxiliary Particle Filter, APF）由Pitt与Shephard于1999年在《美国统计协会杂志》（JASA）上发表的开创性论文中首次提出，该方法通过在重采样前引入观测信息的'前瞻'机制，从根本上革新了序贯蒙特卡罗方法。"
...
假设所有分布关于适当的支配测度（Lebesgue测度或计数测度）绝对连续，则模型可由以下密度函数刻画：

**初始分布密度：** $\mu(x_0) := p(x_0)$

**转移密度（状态转移核）：** $f(x_t | x_{t-1}) := p(x_t | x_{t-1})$

**观测似然密度：** $g(y_t | x_t) := p(y_t | x_t)$
...
### 4.2 Bootstrap滤波器的特殊情形

**定义 4.1（Bootstrap提议分布）** 令 $q(x_t | x_{t-1}, y_t) = f(x_t | x_{t-1})$，即以转移先验为提议分布。
...
输入：归一化权重 $\{W^i\}_{i=1}^N$，其中 $\sum_{i=1}^N W^i = 1$

1. 计算累积分布函数：$C^0 = 0$，$C^i = C^{i-1} + W^i$，$i = 1, \ldots, N$
2. 生成单个均匀随机数：$U \sim \text{Uniform}[0, 1/N]$
3. 设置采样点：$U^i = U + (i-1)/N$，$i = 1, \ldots, N$
4. 对每个 $i$，找到最小的 $j$ 使得 $C^j \geq U^i$，设 $A^i = j$
...
**核心思想：** APF在标准粒子滤波基础上引入离散辅助变量 $K \in \{1, \ldots, N\}$，将滤波问题提升至更高维空间。
...
**定义 5.3（特征值）** 特征值 $\mu_{t+1}^k$ 通常选取为：

**均值：** $\mu_{t+1}^k = \mathbb{E}[X_{t+1} | X_t = x_t^k] = \int x \, f(x | x_t^k) \, dx$

**众数：** $\mu_{t+1}^k = \arg\max_x f(x | x_t^k)$
...
**证明：** 由条件概率定义，$q(x_{t+1}, k) = q(k) \cdot q(x_{t+1} | k)$，两阶段采样正是联合分布的分解形式。 $\square$
...
**输出：** 加权粒子集 $\{(x_t^r, w_t^r)\}_{r=1}^N$，$t = 1, \ldots, T$
...
**命题 8.1（计算复杂度）** 设似然评估复杂度为 $O(C_g)$，转移采样复杂度为 $O(C_f)$，特征值计算复杂度为 $O(C_\mu)$。则：

**标准SIR：** $O(N \cdot (C_f + C_g))$ 每时间步

**APF：** $O(N \cdot (C_\mu + 2C_g + C_f))$ 每时间步

APF的额外开销为特征值计算 $O(N \cdot C_\mu)$ 与第一阶段似然评估 $O(N \cdot C_g)$。
...
辅助粒子滤波通过引入辅助索引变量，实现了观测信息的前瞻性利用。本文详细推导了APF的数学基础，包括联合目标分布的构造、两阶段采样的理论等价性、重要性权重的严格推导，以及收敛性与方差分析。

核心数学结论：

1. **联合分布边缘化**：$\sum_k p(x_{t+1}, k | y_{1:t+1}) \propto \pi_{t+1}(x_{t+1})$，确保APF正确性
2. **权重简化**：由于提议分布设计，第二阶段权重简化为似然比 $g(y_{t+1}|x_{t+1})/g(y_{t+1}|\mu_{t+1}^k)$
3. **修正目标分布**：Johansen-Doucet分析揭示APF实际针对包含前瞻项的修正分布
4. **效率条件**：APF优势取决于第一阶段权重的方差特性与特征值预测质量

理解这些数学细节对于正确实现APF算法、诊断性能问题、以及开发改进变体至关重要。
